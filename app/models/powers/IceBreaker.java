...
public class IceBreaker extends SuperPower {
	public static int CHAT_SECONDS_REQUIRED = 5;
	private static IceBreakers breakers = new IceBreakers();
	...
	public int isQualified (User user) {
		...
		boolean qualified = (user.chatTime == null ? 0 : user.chatTime) - secsUsed > CHAT_SECONDS_REQUIRED;
	...
	public int chooseIndex (User caller, User subject) {
	    ...
	    if (seenIndices.size() == availableCount) {  // one or both has seen every one
