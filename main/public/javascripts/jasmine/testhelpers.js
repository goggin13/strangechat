...
var beforeEachFunction = function () {
  ...
  runs(function () {
    var testLogin = function (JSON, user) {
      ...
      loggedIn = 0;
    user1.login_callback = function (JSON) {
    ...
    user1["api"] = ChatAPI(user1);
    user2.login_callback = function (JSON) {
      testLogin(JSON, user2);
      loggedIn++;
    };
    user2["api"] = ChatAPI(user2); 
    user3.login_callback = function (JSON) {
      testLogin(JSON, user3);
      loggedIn++;
    };      
    user3["api"] = ChatAPI(user3);  
    user4.login_callback = function (JSON) {
      testLogin(JSON, user4);
      loggedIn++;
    };     
    user4["api"] = ChatAPI(user4);
